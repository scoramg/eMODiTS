...
public class TournamentSelection implements ISelector {
    private Map<IScheme,Integer> victories;
    int number_opponents;
    public IPopulation selected;

    public Map<IScheme, Integer> getVictories() {
        return victories;
    }

    public void setVictories(Map<IScheme, Integer> victories) {
    ...
    public TournamentSelection(IPopulation population, int number_opponents) {
        victories = new HashMap<>();
        this.number_opponents = number_opponents;
        Execute(population);
    }

    public IPopulation getSelected() {
        return selected;
    }
...
//                if (population.getIndividuals()[i].getFitnessValue() < population.getIndividuals()[opponent].getFitnessValue()){
        ...
        int w=0;
        for(IScheme individual: this.getVictories().keySet()){
    ...
    public String toString() {
        ...
        for(IScheme v: this.victories.keySet()){
