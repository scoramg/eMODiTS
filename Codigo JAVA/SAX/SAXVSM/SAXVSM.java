 ...
 * @author James
 */
public class SAXVSM extends AbstractClassifier implements Classifier {
...
//        System.out.println("transformedData:"+transformedData.toString());
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        if (useParamSearch) {
//            System.out.println("data.attribute(2).numValues():"+data.attribute(2).numValues());
...
//        System.out.println("bopData.classIndex():"+bopData.classIndex());
    ...
    public void Classify(DataSet ds, boolean UsingTest, String set_type) throws Exception{
        ...
        if(UsingTest){
            ...
            double[][] data = ds.getOriginal().getData();
                
            switch (set_type){
                case "original":
                    data = ds.getOriginal().getData();
                    break;
                case "train":
                    data=ds.getTrain().getData();
                    break;
                case "test":
                    data=ds.getTest().getData();
                    break;
            } 
            ...
            double[] errors = csf.ClassifyByCrossValidation(this);
            this.ErrorRatesByFolds = errors.clone();
            this.ErrorRate = mimath.MiMath.getMedia(errors);
        }
    }
    
//    public void Classify(DiscretizedDataSet ds_dis_train,DiscretizedDataSet ds_dis_test) throws Exception{
//            
...
//    public void Classify(DataSets.Data ds) throws Exception{
//            
//        Classification csf = new Classification(ds.getData());
//        
//        if (csf.getDataSource().classIndex() != csf.getDataSource().numAttributes()-1){
//            Instances PreData = csf.MoveAtts(csf.getDataSource(), 1, csf.getDataSource().numAttributes());
//            PreData = Classification.ConvertNumeric2Nominal(PreData, "last");
...
//            csf.setDataSource(PreData);
//            
////            System.out.println("Data Reorder:"+csf.getDataSource().toString());
//        }
//
//        double[] errors = csf.ClassifyByCrossValidation(this);
//        this.ErrorRate = csf.getErrorRate();
...
//    }
    ...
    public static void main(String[] args){
        try {
            ...
            System.out.println("vsm.ErrorRatesByFolds:");
            for(double d: vsm.ErrorRatesByFolds){
                System.out.println(d);
            }
        } catch (MyException ex) {
            Logger.getLogger(SAXVSM.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(SAXVSM.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
