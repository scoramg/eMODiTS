...
//                        System.out.println(j+","+(b[j]+this.epsilon)+","+(b[j]-this.epsilon));
//                    }
                    if(b[j]+this.epsilon >= a[i] && b[j]-epsilon <=a[i]){
    ...
    public static void main(String[] args) {
        double[] A = {0.24, 1,267, 9.434, 0.6345, 0.98};
        double[] B = {3.24, -1.34, -7.345, -1.645};
        
        double std1 = stdv_s(A);
        double std2 = stdv_s(B);
        double epsilon = Math.min(std1, std2);
        int delta = (int) Math.ceil(Math.max(A.length, B.length)*0.2);
